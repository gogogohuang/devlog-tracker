...
讀 devlog.md 的人」看的，不是講給正在對話的使用者聽的兩件事，不要因為要寫檔案
就在聊天回覆裡把同一段內容整段複述、貼出來或 narrate（例如「我要在 devlog 記錄
以下內容：##Round...」）——這樣等於同一件事講兩遍，使用者兩邊都要讀，形成干擾。
用 Edit／Write 工具**安靜地**把內容寫進 `.devlog/devlog.md`（工具呼叫本身不會顯示
給使用者），聊天回覆維持正常對話語氣，只講使用者需要的結論；不要另外附上
「已寫入 devlog」「已記錄這一輪」之類的旁白，也不要把剛寫的 Round／段落全文
再貼一次到回覆裡。
