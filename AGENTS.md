...
- `.codex-plugin/plugin.json`、`codex/plugin-hooks.json`、`codex/skills/`：Codex plugin 安裝用的 manifest 與內容。改 `codex/hooks.json`、`commands/` 或 `skills/devlog-tracker/` 後，跑 `node scripts/sync-codex-plugin.js` 同步產生檔；`npm test` 會檢查同步狀態。
...
後四個由 `scripts/sync-version.js` 依 `package.json` 自動同步（掛在 `package.json` 的 `"version"` lifecycle script，`pnpm version` 會執行它並把結果併進同一個版本 commit）。
...
Release 一建立（published），`.github/workflows/npm-publish.yml` 就會：檢查 tag 與版號檔案版本一致、跑 `npm test`、用 npm Trusted Publishing（OIDC，不存 token）發布到 npmjs。該版本已經在 npm 上時會直接跳過，不會報錯。
