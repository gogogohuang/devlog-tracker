...
`IN_PROGRESS`、`INTERRUPTED`、`BLOCKED`：先做 `commands/continue.md` **步驟 5.1–5.2** 的工作區核對（不要跟著做 5.4 的立刻開工）。不相符或沒有快照時，在**這一輪**（寫進 `devlog.md` 的 open Round）追加一段 `### 段落`（宣稱 vs 實際）。然後依核對後的**實際工作樹**提出接續方式，**等待使用者確認後才做下一步**：
