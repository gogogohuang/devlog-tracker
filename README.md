...
版本 `0.7.0`。在專案中維護一份 `.devlog/devlog.md`，把每一輪對話的請求、決策與結果寫成永久紀錄。對話一 `/clear` 或換 session 就沒了；這份檔案取代那個缺口，
...
│       ├── segment-watch-set.sh         # /segment-watch：調整 max_silent_seconds
...
└── commands/
    ...
    └── status.md / span.md / segment-watch.md
```
