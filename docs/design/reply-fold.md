...
| File | Role |
|---|---|
| `core/scripts/await-open.sh` | Writes `.devlog/.awaiting-reply` for the current Round |
| `core/scripts/tests/test-await-open.sh` | Self-check for `await-open.sh` |
| `core/scripts/round-start.sh` | Reads/consumes `.awaiting-reply`; folds a matching reply into the last Round instead of opening a new one; also detects and condenses task-notification prompts, folding them the same way |
| `core/scripts/tests/test-round-start.sh` | Self-check covering fold-match, fold-miss, checkpoint-counter behavior, and task-notification condensing/folding |
| `core/scripts/pause-devlog.sh` | Also deletes `.awaiting-reply` on pause |
